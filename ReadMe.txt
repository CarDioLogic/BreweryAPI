Project: Brewery Management App

Description:
This is a project with 3 parts: an API that relies on a repository pattern design and uses Entity framework to interact with a SQLserver database. There are several controllers that enable interaction with all the tables of the database, namely, to perform CRUD operations on Breweries, Beers, Sales, Wholesalers, Wholesaler Inventory and Client Quotes. Then, there is another part (testing) that is responsible to test each method in all the controllers of the API part to ensure they are working properly. And finally a Frontend part (using .Net MAUI) that consumes the API to allow users to interact with the database. 

Technologies:
C#, .Net, .Net MAUI, AutoMapper, Entity Framework, and Microsoft SQL server
